...
function renderNavbar(activePage) {
    ...
    <div id="auth-modal" class="fixed inset-0 z-50 hidden bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 max-w-sm w-full space-y-4 shadow-2xl">
            ...
            <button id="google-login-btn" onclick="loginWithGoogle(event)" class="w-full py-2.5 px-4 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all text-xs font-semibold flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
...
async function loginWithGoogle(e) {
    ...
    if (window.location.protocol === 'file:') {
        if (typeof showToast === 'function') showToast("🌐 Google ile giriş yapabilmek için projenin yerel bir HTTP sunucusunda (Live Server / localhost) veya yayınlanmış domain üzerinde çalışması gerekmektedir.", "warning");
    ...
    // Direct Redirect Yönlendirmeyi Async Await ile Başlat
    try {
        await currentAuth.signInWithRedirect(provider);
    } catch (err) {
        console.error("signInWithRedirect Hatası:", err);
        if (btn) {
            btn.disabled = false;
        ...
        if (typeof showToast === 'function') showToast("Giriş başlatılamadı: " + (err.message || "Bilinmeyen hata"), "error");
    }
}
